...
/// Helper: compute the recall spool number from a given challenge
#[inline(always)]
pub fn compute_recall_spool(
    challenge: &[u8; 32],
    total_spools: u64,
) -> u64 {
    ...
    // Compute the spool number from the challenge, spool 0 
    // is invalid and reprseents no spool
    u64::from_le_bytes(challenge[0..8].try_into().unwrap()) % total_spools + 1
}
