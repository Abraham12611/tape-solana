...
use spool_network::store::SpoolStore;
use std::env;
use std::str::FromStr;
...
pub enum Commands {
    ...
    Read {
        #[arg(help = "Spool account to read")]
        spool: String,
    ...
    Write {
        ...
        #[arg(short = 'n', long = "spool-name", help = "Custom name for the spool (defaults to timestamp)")]
...
pub enum SnapshotCommands {
    ...
    Resync {
        #[arg(help = "Spool account public key to re-sync")]
    ...
    GetSpool {
        ...
        #[arg(short = 'r', long = "raw", help = "Output raw segments instead of decoded spool")]
        raw: bool,
    ...
    GetSegment {
        ...
        #[arg(help = "Segment index (0 to spool size - 1)")]
        index: u32,
...
impl Context{
    ...
    pub fn open_primary_store_conn(&self) -> Result<SpoolStore> {
        Ok(spool_network::store::primary()?)
    }

    pub fn open_secondary_store_conn_mine(&self) -> Result<SpoolStore> {
        Ok(spool_network::store::secondary_mine()?)
    }

    pub fn open_secondary_store_conn_web(&self) -> Result<SpoolStore> {
        Ok(spool_network::store::secondary_web()?)
    }

    pub fn open_read_only_store_conn(&self) -> Result<SpoolStore> {
        Ok(spool_network::store::read_only()?)
    }
