...
pub fn spool_bytes() -> Vec<u8> {
    // Fetch the spool program bytes from target
    let mut so_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    so_path.push("../target/deploy/spool.so");
    std::fs::read(so_path).unwrap()
}
...
pub fn setup_svm() -> LiteSVM {
    ...
    svm.add_program(spool_api::ID, &spool_bytes());
    svm.add_program(example::ID, &program_bytes());

    let payer = create_payer(&mut svm);
    let payer_pk = payer.pubkey();

    // Initialize the spool program (not usually required, this would already be done on-chain)
    init_spool_program(&mut svm, &payer);
