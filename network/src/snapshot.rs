...
pub fn create_snapshot(db: &DB, archive_path: impl AsRef<Path>) -> Result<(), StoreError> {
    ...
    let temp_dir = TempDir::new("spoolstore")?;
...
pub fn load_from_snapshot(archive_path: impl AsRef<Path>, extract_to: impl AsRef<Path>) -> Result<SpoolStore, StoreError> {
    let archive_path = archive_path.as_ref();
