...
use spool_api::consts::PACKED_SEGMENT_SIZE;
use crate::store::*;
use crate::metrics::inc_total_segments_written;

pub trait SegmentOps {
    fn put_segment(&self, spool_address: &Pubkey, global_seg_idx: u64, seg: Vec<u8>) -> Result<(), StoreError>;
    fn get_segment(&self, spool_address: &Pubkey, global_seg_idx: u64) -> Result<Vec<u8>, StoreError>;
    fn get_segment_range(&self, spool_address: &Pubkey, start: u64, end: u64) -> Result<Vec<(u64, Vec<u8>)>, StoreError>;
    fn get_spool_segments(&self, spool_address: &Pubkey) -> Result<Vec<(u64, Vec<u8>)>, StoreError>;
    fn get_segment_count(&self, spool_address: &Pubkey) -> Result<usize, StoreError>;
}

impl SegmentOps for SpoolStore {
    fn get_segment(&self, spool_address: &Pubkey, segment_number: u64) -> Result<Vec<u8>, StoreError> {
        let cf = self.get_cf_handle(ColumnFamily::Segments)?;
        let mut key = Vec::with_capacity(40);
        key.extend_from_slice(&spool_address.to_bytes());
        key.extend_from_slice(&segment_number.to_be_bytes());
        let segment_data = self
            .db
            .get_cf(&cf, &key)?
            .ok_or(StoreError::SegmentNotFound(spool_address.to_string(), segment_number))?;
        Ok(segment_data)
    }

    fn put_segment(&self, spool_address: &Pubkey, segment_number: u64, data: Vec<u8>) -> Result<(), StoreError> {
    ...
    fn get_segment_range(&self, spool_address: &Pubkey, start: u64, end: u64) -> Result<Vec<(u64, Vec<u8>)>, StoreError> {
        ...
        let cf = self.get_cf_handle(ColumnFamily::Segments)?;
        let prefix = spool_address.to_bytes().to_vec();
        ...
        for item in iter {
            ...
            let segment_number = u64::from_be_bytes(
                key[32..SPOOL_STORE_SLOTS_KEY_SIZE]
                    .try_into()
...
mod tests {
    ...
    #[test]
    fn test_get_spool_segments() -> Result<(), StoreError> {
        let (store, _temp_dir) = setup_store()?;
        let address = Pubkey::new_unique();
        ...
        let segments = store.get_spool_segments(&address)?;
    ...
    fn test_get_segment_range() -> Result<(), StoreError> {
        ...
        // Test full range equivalent to get_spool_segments
