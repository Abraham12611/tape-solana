...
impl StatsOps for SpoolStore {
    fn get_local_stats(&self) -> Result<LocalStats, StoreError> {
        let spools = self.count_spools()?;
        let sectors = self.count_sectors()?;
        let size_bytes = self.db_size()?;
        Ok(LocalStats { spools, sectors, size_bytes })
    }

}

impl SpoolStore {
    fn count_spools(&self) -> Result<usize, StoreError> {
...
mod tests {
    ...
    use tempdir::TempDir;
    use spool_api::PACKED_SEGMENT_SIZE;
