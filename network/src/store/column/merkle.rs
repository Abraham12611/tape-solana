...
pub enum MerkleCacheKey {
    ...
    UnpackedSpoolLayer  { address: Pubkey, layer: u8 },   // Unpacked spool layer nodes
    PackedSpoolLayer { address: Pubkey, layer: u8 },      // Packed spool layer nodes
    Raw36([u8; 36]),
}
...
impl MerkleCacheKey {
    ...
    fn to_vec(&self) -> Vec<u8> {
        match *self {
            ...
            MerkleCacheKey::UnpackedSpoolLayer { address, layer } =>
                build_key(&address, SPOOL_LAYER, layer),

            MerkleCacheKey::PackedSpoolLayer { address, layer } =>
...
mod tests {
    ...
    fn setup_store() -> Result<(SpoolStore, TempDir), StoreError> {
        ...
        let store = SpoolStore::new(tmp.path())?;
        Ok((store, tmp))
    }
    ...
    fn merkle_unpacked_layer_roundtrip_vec() -> Result<(), StoreError> {
        ...
        store.put_merkle_cache(
            &MerkleCacheKey::UnpackedSpoolLayer { address: addr, layer },
            &vals,
        )?;
    ...
    fn merkle_packed_layer_roundtrip_vec() -> Result<(), StoreError> {
        ...
        store.put_merkle_cache(
            &MerkleCacheKey::PackedSpoolLayer { address: addr, layer },
            &vals,
        )?;
    ...
    fn merkle_raw36_key_roundtrip() -> Result<(), StoreError> {
        ...
        let addr = Pubkey::new_unique();
        let raw_key: [u8; 36] = {
            let v = build_key(&addr, SPOOL_LAYER, 7);
            let mut arr = [0u8; 36];
    ...
    fn merkle_not_found_returns_error() -> Result<(), StoreError> {
        ...
        let res = store.get_merkle_cache(
            &MerkleCacheKey::UnpackedSpoolLayer { address: addr, layer: 0 },
        );
    ...
    fn merkle_invalid_stored_length_is_rejected() -> Result<(), StoreError> {
        let (store, _tmp) = setup_store()?;
        let addr = Pubkey::new_unique();
        let key_vec = build_key(&addr, SPOOL_LAYER, 3);
        ...
        let res = store.get_merkle_cache(
            &MerkleCacheKey::UnpackedSpoolLayer { address: addr, layer: 3 },
        );
    ...
    fn merkle_fixed_len_mismatch_is_rejected() -> Result<(), StoreError> {
        ...
        store.put_merkle_cache(
            &MerkleCacheKey::PackedSpoolLayer { address: addr, layer: 1 },
            &vals,
        )?;
