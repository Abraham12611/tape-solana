...
impl ColumnFamily {
    pub fn as_str(&self) -> &'static str {
        match self {
            ColumnFamily::SpoolByNumber => "spool_by_number",
            ColumnFamily::SpoolByAddress => "spool_by_address",
...
pub fn create_cf_descriptors() -> Vec<ColumnFamilyDescriptor> {
    let column_families = [
        (ColumnFamily::SpoolByNumber, 8, Some(plain_table_options(8)), None),
        (ColumnFamily::SpoolByAddress, 32, Some(plain_table_options(32)), None),
        (ColumnFamily::Segments, 32, None, Some(block_based_options())),
