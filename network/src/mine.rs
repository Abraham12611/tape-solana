...
use spool_client::mine::mine::perform_mining;
...
use crate::metrics::{
    inc_spool_mining_attempts_total, 
    inc_spool_mining_challenges_solved_total, 
    observe_spool_mining_duration, 
    run_metrics_server, 
...
async fn try_mine_iteration(
    ...
    let res = store.get_spool_address(spool_number);
    if res.is_err() {
        debug!("Spool address not found in local db, nothing to do for now...");
        return Ok(());
    }

    let spool_address = res.unwrap();
    debug!("Spool address: {spool_address:?}");

    let (spool, _) = get_spool_account(client, &spool_address)
        .await
        .map_err(|e| anyhow!("Failed to get spool account: {}", e))?;
    ...
    if spool.has_minimum_rent() {
        ...
        let merkle_tree = SegmentTree::new(&[spool_address.as_ref()]);
        ...
        let segment = store.get_segment(&spool_address, segment_number)?;
        ...
        // TODO: leaving this out for now, as it requires managing miner reels
            ...
            loop {
                ...
                if let Ok(solution) = solve_with_memory(
                    ...
                    if solution.difficulty() >= difficulty as u32 {
                        ...
                        observe_spool_mining_duration(elapsed.as_secs_f64());
