...
use spool_api::instruction::spool::build_set_header_ix;
...
pub const HEADER_MAGIC: [u8; 4] = *b"SPOL";
...
/// Flags for the spool data header.
pub enum SpoolFlags {

    /// No flags set (use this if you're producing a spool entirely on-chain, not uploading a file).
    None = 0,

    /// Store the spool data, prefixed with the segment number (u64). Each write can have multiple
    ...
    ///
    /// You should probably set this if you're uploading a file to a spool and want to write it
    /// quickly.
    Prefixed = 1 << 0,
...
/// Spool data header. Note, none of the fields are verified onchain, they are 
...
impl SpoolHeader {
    ...
    pub fn try_from_bytes(data: &[u8]) -> Result<&Self> {
        ...
        if data.len() < std::mem::size_of::<Self>() {
            bail!("Data too short for SpoolHeader ({} < {})",
                  data.len(), std::mem::size_of::<Self>());
        }

        // Check the magic prefix.
        if data[0..4] != HEADER_MAGIC {
            bail!("Invalid magic number in SpoolHeader");
        }

        // Check the version byte.
        if data[4] != HEADER_VERSION {
            bail!(
                "Unsupported SpoolHeader version: found {}, expected {}",
        ...
        let header: &Self = bytemuck::try_from_bytes(data)
            .map_err(|e| anyhow!("Failed to cast bytes to SpoolHeader: {}", e))?;

        Ok(header)
    }
}

impl std::fmt::Debug for SpoolHeader {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SpoolHeader")
...
/// Sets the spool header for a given spool account.
...
mod tests {
    ...
    #[test]
    fn test_spool_header_creation() {
        let header = SpoolHeader::new(
            MimeType::ImagePng,
        ...
        assert_eq!(header.flags, SpoolFlags::None as u8);
    ...
    fn test_spool_header_to_bytes() {
        ...
        assert_eq!(bytes.len(), std::mem::size_of::<SpoolHeader>());
    }
}
