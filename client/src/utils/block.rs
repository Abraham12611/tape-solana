...
use spool_api::instruction::spool::{SpoolInstruction, Update};
...
pub struct ProcessedBlock {
    pub slot: u64,
    pub finalized_spools: HashMap<Pubkey, u64>,
    pub segment_writes: HashMap<SegmentKey, Vec<u8>>,
}

#[derive(Debug, Default)]
struct MergedSpoolData {
    finalized_spools: HashMap<Pubkey, u64>,
...
pub fn process_block(block: UiConfirmedBlock, slot: u64) -> Result<ProcessedBlock, BlockError> {
    ...
    let mut spool_block = SpoolBlock::default();

    for tx in transactions {
        process_transaction(&tx, &mut spool_block)?;
    }

    verify_counts(&spool_block)?;
    let merged = merge_events_and_instructions(&spool_block)?;

    Ok(ProcessedBlock {
        slot,
        finalized_spools: merged.finalized_spools,
        segment_writes: merged.segment_writes,
    })
}

fn verify_counts(spool_block: &SpoolBlock) -> Result<(), BlockError> {
    let (mut write_events, mut update_events, mut finalize_events) = (0, 0, 0);
    for event in &spool_block.events {
        match event {
    ...
    for ix in &spool_block.instructions {
        match ix {
    ...
    if spool_block.events.len() != spool_block.instructions.len() {
...
fn merge_events_and_instructions(
    spool_block: &SpoolBlock,
) -> Result<MergedSpoolData, BlockError> {
    let mut merged = MergedSpoolData::default();

    // Iterate over events and instructions in parallel
    for (event, instruction) in spool_block.events.iter().zip(&spool_block.instructions) {
...
fn merge_finalize(
    ...
    merged.finalized_spools.insert(*address, finalize_event.spool);
        ...
        UiMessage::Raw(raw_message) => {
            if let Some(meta) = &tx.meta {
                if let OptionSerializer::Some(log_messages) = &meta.log_messages {
                    process_log_messages(log_messages, spool_block)?;
                }
            }
            ...
            process_inner_instructions(&raw_message.account_keys, &tx.meta, spool_block)?;
            Ok(())
        }
        _ => {
            Ok(())
...
fn process_log_messages(
    ...
    let events = &mut spool_block.events;
    ...
    for log in log_messages {
        ...
        let is_spool_program = program_stack.last() == Some(&spool_api::ID);

        if is_spool_program && is_program_data(log) {
            let event_data =
...
fn process_top_level_instructions(
    ...
    for ix in instructions {
        ...
        if program_id == spool_api::ID {
            let spool_ix = process_instruction(ix, account_keys)?;
            if let Some(ix) = spool_ix {
                spool_block.instructions.push(ix);
            }
        }
    }
        ...
        for inner_ix in &inner_ix_set.instructions {
            if let UiInstruction::Compiled(compiled_ix) = inner_ix {
                ...
                let program_id = account_keys[program_id_index]
                    ...
                    .map_err(|_| BlockError::InvalidPubkey)?;
                if program_id == spool_api::ID {
                    let spool_ix = process_instruction(compiled_ix, account_keys)?;
                    if let Some(ix) = spool_ix {
                        spool_block.instructions.push(ix);
                    }
                }
...
fn process_instruction(
    ...
    let spool_index = *ix
        .accounts
        .get(1)
        .ok_or(BlockError::InvalidData("Missing spool account"))? as usize;

    if spool_index >= account_keys.len() {
        return Err(BlockError::InvalidData("Invalid spool account index"));
    }

    let spool_address = account_keys[spool_index]
        .parse::<Pubkey>()
        .map_err(|_| BlockError::InvalidPubkey)?;
    ...
    let ix_type = SpoolInstruction::try_from(ix_data[0]);
    if ix_type.is_err() {
        return Ok(None);
    }

    match ix_type.unwrap() {
        SpoolInstruction::Write => Ok(Some(InstructionData::Write {
            address: spool_address,
            data: ix_data[1..].to_vec(),
        })),
        SpoolInstruction::Update => {
        ...
        SpoolInstruction::Finalize => Ok(Some(InstructionData::Finalize {
            address: spool_address,
        })),
        _ => Ok(None),
    }
